...
public class EmailPreferenceUtil {
	...
	@Bean
	public List<EmailPreferences> readEmailPreferenceFile() {
		
		List<EmailPreferences> emailPreferenceList = new ArrayList<>();
		try {
			...
			while (sc.hasNext()) {
				...
				EmailPreferences emailPrefObj = new EmailPreferences();
	...
	public List<EmailPreferences> readFromXlsxFile() {
	        ...
	        dataSheet.forEach(row -> {
	            ...
	            row.forEach(cell -> {
	                ...
	                switch (cell.getColumnIndex()) {
	                	...
	                	case 6:
	                		emailPrefObj.setBookInd(cellValue);
